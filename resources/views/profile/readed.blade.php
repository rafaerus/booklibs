...
<head>
    ...
    <title>Books Read - Booklibs</title>
        ...
        <div class="flex-1 flex flex-col items-center px-6 py-8">
            <div class="w-full max-w-7xl">
                <div class="flex justify-between items-center mb-8">
                    <h1 class="text-2xl font-semibold text-gray-900">Books Read</h1>
                    <div class="flex items-center gap-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                    @forelse ($readedBooks as $history)
                    <div class="group">
                        <div class="aspect-[3/4] rounded-lg overflow-hidden bg-gray-100">
                            <img src="{{ $history->book->cover_url ?? 'https://images.unsplash.com/photo-1544947950-fa07a98d237f' }}" alt="Book Cover" class="w-full h-full object-cover">
                        </div>
                        <div class="mt-2">
                            ...
                            <p class="text-xs text-gray-400 mt-1">Last read: {{ $history->last_read ? \Carbon\Carbon::parse($history->last_read)->diffForHumans() : '-' }}</p>
                        </div>
                        <form method="POST" action="{{ route('save.book') }}" class="w-full mt-2">
                            @csrf
