...
<div class="container mx-auto px-4 py-8">
    ...
    <form action="{{ route('books.update', $book) }}" method="POST" class="max-w-lg">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" id="title" value="{{ old('title', $book->title) }}" class="w-full border border-gray-300 rounded px-3 py-2" required>
            @error('title')
        ...
        <div class="mb-4">
            ...
            <input type="text" name="author" id="author" value="{{ old('author', $book->author) }}" class="w-full border border-gray-300 rounded px-3 py-2" required>
            @error('author')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="isbn" id="isbn" value="{{ old('isbn', $book->isbn) }}" class="w-full border border-gray-300 rounded px-3 py-2" required>
            @error('isbn')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="published_at" id="published_at" value="{{ old('published_at', $book->published_at) }}" class="w-full border border-gray-300 rounded px-3 py-2" required>
            @error('published_at')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Update Book</button>
    </form>
</div>
@endsection
